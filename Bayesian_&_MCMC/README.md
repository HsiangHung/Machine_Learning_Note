...
<a href="http://www.codecogs.com/eqnedit.php?latex=P(x_i,&space;y_i|\theta)&space;\sim&space;\exp{\frac{(y_i-\theta&space;x_i)^2}{2\sigma^2}}" target="_blank"><img src="http://latex.codecogs.com/gif.latex?P(x_i,&space;y_i|\theta)&space;\sim&space;\exp{\frac{(y_i-\theta&space;x_i)^2}{2\sigma^2}}" title="P(x_i, y_i|\theta) \sim \exp{\frac{(y_i-\theta x_i)^2}{2\sigma^2}}" /></a>
...
<a href="http://www.codecogs.com/eqnedit.php?latex=\theta_{\textrm{MLE}}&space;=&space;\underset{\theta}{\textrm{argmax}}&space;P(D|\theta)&space;=&space;\underset{\theta}{\textrm{argmax}}&space;\Big(&space;\theta^{N_H}(1-\theta)^{N_T}&space;\Big)&space;=&space;\underset{\theta}{\textrm{argmax}}&space;\&space;l(\theta)" target="_blank"><img src="http://latex.codecogs.com/gif.latex?\theta_{\textrm{MLE}}&space;=&space;\underset{\theta}{\textrm{argmax}}&space;P(D|\theta)&space;=&space;\underset{\theta}{\textrm{argmax}}&space;\Big(&space;\theta^{N_H}(1-\theta)^{N_T}&space;\Big)&space;=&space;\underset{\theta}{\textrm{argmax}}&space;\&space;l(\theta)" title="\theta_{\textrm{MLE}} = \underset{\theta}{\textrm{argmax}} P(D|\theta) = \underset{\theta}{\textrm{argmax}} \Big( \theta^{N_H}(1-\theta)^{N_T} \Big) = \underset{\theta}{\textrm{argmax}} \ l(\theta)" /></a>

To do it, we differentiate l(θ) with respect to θ and make it equal to zero. Then we obtain
