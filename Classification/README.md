...
$$L(\theta, \symbf{x}) = - \sum^K_{j=1}\symbf{I}(y=j)\log\big( h_{\theta}(\symbf{x}) \big) = - \sum^K_{j=1} \symbf{I}(y=j)\log\big( \frac{e^{\theta^{(j)T}\symbf{x}}}{ \sum_{j=1}^K e^{\theta^{(j)T}\symbf{x}}} \big)$$
