...
$$C(\symbf{\theta}) = C \sum^m_{i=1} y_i \textrm{Cost}_1(\theta^T \symbf{x}_i) + (1-y_i) \textrm{Cost}_0(\theta^T \symbf{x}_{i} ) + \frac{1}{2} \sum |\theta_j| ^2$$
...
$$C(\symbf{\theta}) = C \sum^m_{i=1} \Big( y_i \max{\big(0, 1- \theta^T \symbf{x}_i)} + (1-y_i) \max{\big(0, 1 + \theta^T \bf{x}_i)} \Big) + \frac{1}{2}\sum^n_{j=1} |\theta_j|^2$$
