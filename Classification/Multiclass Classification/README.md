...
$$L(\theta, \mathbf{x}) = - \sum^K_{j=1} \mathbb{I}(y=j)\log\big( h_{\theta}(\mathbb{x}) \big) = - \sum^K_{j=1} \mathbf{I}(y=j)\log \left( \frac{\exp (\theta^{(j)^T} \mathbb{x}) }{ \sum_j \exp (\theta^{(j)^T} \mathbf{x} ) } \right),$$

where $\mathbb{I} = 1$ for $y = j$; otherwise $\mathbb{I}=0$. 
