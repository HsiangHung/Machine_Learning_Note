...
$$L(\theta, \mathbf{x}) = - \sum^K_{j=1} \mathbf{I}(y=j)\log\big( h_{\theta}(\mathbf{x}) \big) = - \sum^K_{j=1} \mathbf{I}(y=j)\log \left( \frac{\exp (\theta^{(j)T}\mathbf{x}) }{ \sum_j^K \exp (\theta^{(j)T}\mathbf{x} ) } \right) $$
