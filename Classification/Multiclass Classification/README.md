...
$$L(\theta, \mathbf{x}) = - \sum^K_{j=1} \mathbf{I}(y=j)\log\big( h_{\theta}(\mathbf{x}) \big) = - \sum^K_{j=1} \mathbf{I}(y=j)\log \Big( \frac{\exp{\theta^{(j)T}\mathbf{x}}}{ \sum^K_j e^{\theta^{(j)T}\mathbf{x}}} \Big) $$
