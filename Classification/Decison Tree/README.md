...
Then we can calculate expected **Expected Entropy** (EH) remaining after trying attribute `A` (with branches i=1,2,...,K) is
...
The information gain is `E(p,n) - EH(A)` for attribute `A`.
